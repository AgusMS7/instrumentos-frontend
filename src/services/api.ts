...
export const instrumentoService = {
  ...
  getAll: async (page = 0, size = 10, sort = "id,asc") => {
    const response = await fetch(`${API_BASE_URL}/instrumentos?page=${page}&size=${size}&sort=${sort}`)
  ...
  getByCategoria: async (categoriaId: number) => {
    const response = await fetch(`${API_BASE_URL}/instrumentos/categoria/${categoriaId}`)
    if (!response.ok) throw new Error(`Error ${response.status}: ${response.statusText}`)
    return response.json()
  },
  ...
  search: async (nombre: string) => {
    const response = await fetch(`${API_BASE_URL}/instrumentos/buscar?nombre=${encodeURIComponent(nombre)}`)
    if (!response.ok) throw new Error(`Error ${response.status}: ${response.statusText}`)
    return response.json()
  },
}
...
export const imageService = {
  // Subir imagen
  upload: async (instrumentoId: number, file: File, altText = "", isPrimary = false) => {
    const formData = new FormData()
    formData.append("file", file)
    formData.append("altText", altText)
    formData.append("isPrimary", isPrimary.toString())

    const response = await fetch(`${API_BASE_URL}/images/upload/${instrumentoId}`, {
      method: "POST",
      body: formData,
    })
    if (!response.ok) throw new Error(`Error ${response.status}: ${response.statusText}`)
  ...
  getByInstrumento: async (instrumentoId: number) => {
    const response = await fetch(`${API_BASE_URL}/images/instrumento/${instrumentoId}`)
    if (!response.ok) throw new Error(`Error ${response.status}: ${response.statusText}`)
    return response.json()
  },
  ...
  getPrimary: async (instrumentoId: number) => {
    const response = await fetch(`${API_BASE_URL}/images/instrumento/${instrumentoId}/primary`)
    if (!response.ok) throw new Error(`Error ${response.status}: ${response.statusText}`)
    return response.json()
  },
